...
fn list_windows_current_space(_hide_system_windows: bool) -> Vec<NativeWindow> {
    unsafe {
        ...
        for i in 0..count {
            ...
            let title =
                cf_dict_get_string(dict_ptr, key_name.as_concrete_TypeRef()).unwrap_or_default();
...
unsafe fn ax_copy_attribute(elem: AXUIElementRef, attribute: &CFString) -> Option<CFTypeRef> {
    ...
    unsafe {
        ...
        for i in 0..count {
            ...
            if hide_system_windows {
                ...
                let bounds_ptr =
                    CFDictionaryGetValue(dict_ptr, key_bounds.as_concrete_TypeRef() as *const _)
                        as CFDictionaryRef;
                ...
                let alpha =
                    cf_dict_get_f64(dict_ptr, key_alpha.as_concrete_TypeRef()).unwrap_or(1.0);
                if alpha <= 0.01 {
                    continue;
                }
...
fn raise_ax_window_by_cg_id(pid: u32, target_wid: u32) -> bool {
    ...
    with_ax_window_by_cg_id(pid, target_wid, |win| {
        let raise_err = unsafe {
            AXUIElementPerformAction(win, action_raise.as_concrete_TypeRef() as CFStringRef)
        };
